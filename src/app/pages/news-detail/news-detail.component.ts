...
export class NewsDetailComponent {
    ...
    effect(() => {
      ...
      if (news) {
        this.seo.updateMetaForDynamicContent({
          ...
          ogImage: news.image ? this.siteService.getImageUrl(news, news.image) : 'default-news-image.jpg'
        });
  ...
  private async loadRelatedNews(tag: string, currentNewsId: string) {
    const related = await this.pocketbase.getRelatedNews(tag, currentNewsId);
